<template>
  <div class="min-h-screen bg-[#0A0A0A] text-white">
    <aside class="fixed left-0 top-0 h-full w-64 bg-[#0F0F0F] border-r border-white/[0.06] z-20 flex flex-col">
      ...
      <nav class="flex-1 p-4 flex flex-col gap-1.5">
        ...
        <button @click="activeTab = 'products'"
          :class="[activeTab === 'products' ? 'bg-[#D4AF37]/10 border-[#D4AF37]/20 text-[#D4AF37]' : 'text-white/40 border-transparent hover:bg-white/[0.03] hover:text-white', 'flex items-center gap-3 px-4 py-3 rounded-xl border transition-all duration-300 group']">
          ...
          <span class="ml-auto bg-white/5 text-[8px] px-1.5 py-0.5 rounded group-hover:bg-[#D4AF37]/20">{{
            products.length }}</span>
        </button>

        <button @click="activeTab = 'add'; editMode = false; resetForm()"
          :class="[activeTab === 'add' ? 'bg-[#D4AF37]/10 border-[#D4AF37]/20 text-[#D4AF37]' : 'text-white/40 border-transparent hover:bg-white/[0.03] hover:text-white', 'flex items-center gap-3 px-4 py-3 rounded-xl border transition-all duration-300']">
        ...
        <button @click="activeTab = 'orders'"
          :class="[activeTab === 'orders' ? 'bg-[#D4AF37]/10 border-[#D4AF37]/20 text-[#D4AF37]' : 'text-white/40 border-transparent hover:bg-white/[0.03] hover:text-white', 'flex items-center gap-3 px-4 py-3 rounded-xl border transition-all duration-300 group']">
          <Icon name="lucide:shopping-bag" class="w-4 h-4" />
          <span class="font-montserrat text-[9px] font-bold tracking-widest uppercase">Orders</span>
          <span class="ml-auto bg-red-500/20 text-red-400 text-[8px] px-1.5 py-0.5 rounded-full">{{ orders.length
            }}</span>
        </button>
      </nav>
      ...
      <div class="p-4 border-t border-white/[0.06] bg-white/[0.01]">
        <div class="flex items-center gap-3 px-3 py-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
          <div class="w-8 h-8 rounded-full bg-gradient-to-tr from-[#D4AF37] to-[#F5E6AD] p-[1px]">
            <div
              class="w-full h-full rounded-full bg-[#0F0F0F] flex items-center justify-center font-bold text-[#D4AF37] text-[10px]">
              A</div>
          </div>
    ...
    <main class="ml-64 min-h-screen">
      <header
        class="sticky top-0 z-10 bg-[#0A0A0A]/90 backdrop-blur-2xl border-b border-white/[0.06] px-10 py-6 flex items-center justify-between">
        <div>
          <h1 class="font-cormorant italic text-2xl font-semibold tracking-tight">
            {{ activeTab === 'products' ? 'Catalog Overview' : activeTab === 'orders' ? 'Order Management' : editMode ?
              'Modify Piece' : 'Create Masterpiece' }}
          </h1>
      ...
      <div class="px-10 py-8">
        <div class="grid grid-cols-3 gap-5 mb-10">
          <div v-for="(stat, i) in stats" :key="i"
            class="bg-gradient-to-b from-white/[0.04] to-transparent border border-white/[0.06] rounded-2xl p-5 hover:border-[#D4AF37]/30 transition-all duration-500">
            <p class="font-montserrat text-[8px] font-bold tracking-[0.2em] uppercase text-white/20 mb-3">{{ stat.label
              }}</p>
            <div class="flex items-baseline gap-2">
              <p class="font-cormorant italic text-3xl font-semibold">{{ stat.value }}</p>
              <span v-if="stat.suffix" class="font-montserrat text-[8px] text-[#D4AF37] font-bold">{{ stat.suffix
                }}</span>
            </div>
          </div>
        </div>
        ...
        <transition name="slide-up">
          <div v-if="activeTab === 'add'"
            class="max-w-4xl bg-[#111] border border-white/[0.08] rounded-[24px] p-8 mb-10 shadow-2xl">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div class="flex flex-col gap-4">
                <label class="font-montserrat text-[8px] font-bold tracking-[0.3em] uppercase text-white/30">Visual
                  Identity</label>
                <div @click="$refs.fileInput.click()"
                  class="relative aspect-square rounded-[20px] border border-dashed border-white/[0.08] hover:border-[#D4AF37]/40 bg-white/[0.02] flex flex-col items-center justify-center cursor-pointer transition-all overflow-hidden group">
                  <img v-if="previewImage" :src="previewImage"
                    class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                  <div v-else class="flex flex-col items-center gap-3">
                    <Icon name="lucide:image-plus"
                      class="w-6 h-6 text-white/10 group-hover:text-[#D4AF37] transition-colors" />
                    <span class="font-montserrat text-[8px] font-bold tracking-widest uppercase text-white/20">Upload
                      Photo</span>
                  </div>
              ...
              <div class="flex flex-col gap-5">
                <div class="space-y-4">
                  <div>
                    <label
                      class="font-montserrat text-[8px] font-bold tracking-[0.2em] uppercase text-white/40 block mb-2">Product
                      Title</label>
                    <input v-model="form.name"
                      class="w-full bg-white/[0.04] border border-white/10 rounded-xl px-4 py-3 font-montserrat text-[11px] focus:border-[#D4AF37]/50 transition-all outline-none"
                      placeholder="e.g. Peshawari Zalmi Gold" />
                  </div>
                  <div>
                    <label
                      class="font-montserrat text-[8px] font-bold tracking-[0.2em] uppercase text-white/40 block mb-2">Valuation
                      (PKR)</label>
                  ...
                  <div>
                    <label
                      class="font-montserrat text-[8px] font-bold tracking-[0.2em] uppercase text-white/40 block mb-2">Story
                      / Description</label>
                    <textarea v-model="form.description" rows="3"
                      class="w-full bg-white/[0.04] border border-white/10 rounded-xl px-4 py-3 font-montserrat text-[11px] focus:border-[#D4AF37]/50 transition-all outline-none resize-none"
                      placeholder="Craftsmanship details..."></textarea>
                  </div>
                </div>

                <div class="flex gap-3 mt-2">
                  <button @click="saveProduct" :disabled="saving"
                    class="flex-1 bg-[#D4AF37] text-black py-3.5 rounded-xl font-montserrat text-[9px] font-bold tracking-[0.3em] uppercase hover:bg-[#F5E6AD] transition-all disabled:opacity-50">
                    {{ saving ? 'Processing...' : editMode ? 'Commit Changes' : 'Confirm Product' }}
                  </button>
                  <button v-if="editMode" @click="cancelEdit"
                    class="px-6 border border-white/10 rounded-xl font-montserrat text-[9px] font-bold tracking-[0.3em] uppercase hover:bg-red-500/10 hover:text-red-400 transition-all">Cancel</button>
                </div>
              </div>
            </div>

            <transition name="fade">
              <div v-if="formMsg.text"
                :class="[formMsg.type === 'success' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20', 'mt-6 p-3 rounded-xl border text-center font-montserrat text-[9px] font-bold uppercase tracking-widest']">
                {{ formMsg.text }}
              </div>
            </transition>
          </div>
        ...
        <transition name="fade">
          <div v-if="activeTab === 'products'"
            class="bg-[#111] border border-white/[0.08] rounded-[24px] overflow-hidden shadow-2xl">
            <div class="px-6 py-4 border-b border-white/[0.06] flex items-center justify-between bg-white/[0.01]">
              <p class="font-montserrat text-[9px] font-bold tracking-[0.3em] uppercase text-white/40">Vault Inventory
              </p>
            </div>
              ...
              <table class="w-full text-left">
                <thead>
                  <tr class="text-white/20 border-b border-white/[0.04]">
                    <th class="px-6 py-4 font-montserrat text-[8px] font-bold uppercase tracking-widest">Masterpiece
                    </th>
                    <th class="px-6 py-4 font-montserrat text-[8px] font-bold uppercase tracking-widest">Market Value
                    </th>
                    <th class="px-6 py-4 font-montserrat text-[8px] font-bold uppercase tracking-widest text-right">
                      Operations</th>
                  </tr>
                </thead>
                  ...
                  <tr v-for="product in products" :key="product.id" class="group hover:bg-white/[0.01] transition-all">
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-4">
                        ...
                        <div>
                          <p
                            class="font-cormorant text-lg font-bold group-hover:text-[#D4AF37] transition-colors leading-none">
                            {{ product.name }}</p>
                    ...
                    <td class="px-6 py-4">
                      <span class="font-montserrat text-xs font-bold text-white">PKR {{
                        Number(product.price).toLocaleString() }}</span>
                    </td>
                    <td class="px-6 py-4">
                      <div class="flex justify-end gap-2">
                        <button @click="editProduct(product)"
                          class="p-2.5 rounded-lg bg-white/[0.03] border border-white/10 hover:text-[#D4AF37] transition-all">
                          <Icon name="lucide:edit-3" class="w-3.5 h-3.5" />
                        </button>
                        <button @click="deleteProduct(product.id)"
                          class="p-2.5 rounded-lg bg-white/[0.03] border border-white/10 hover:text-red-500 transition-all">
        ...
        <transition name="slide-up">
          <div v-if="activeTab === 'orders'"
            class="bg-[#111] border border-white/[0.06] rounded-2xl overflow-hidden shadow-2xl">
              ...
              <div v-for="order in orders" :key="order.id" class="p-6 hover:bg-white/[0.01] transition-colors">
                <div class="flex items-start justify-between mb-4">
                  ...
                  <div class="text-right">
                    <p class="font-montserrat text-lg font-black text-[#D4AF37]">Rs. {{
                      Number(order.total_price).toLocaleString() }}</p>
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                  <div v-for="item in order.items" :key="item.id"
                    class="flex items-center gap-2 bg-white/[0.02] border border-white/[0.05] rounded-lg px-3 py-2">
                    ...
                    <div>
                      ...
                      <p class="font-montserrat text-[7px] text-white/25">UK {{ item.size }} · Qty {{ item.quantity }}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                  <select :value="order.status" @change="updateStatus(order.id, $event.target.value)"
                    class="bg-white/[0.04] border border-white/[0.08] text-white/60 rounded-lg px-3 py-2 font-montserrat text-[8px] uppercase tracking-widest outline-none">
                    <option v-for="s in statusOptions" :key="s" :value="s" class="bg-[#111]">{{ s }}</option>
                  </select>
                  <button @click="deleteOrder(order.id)"
                    class="flex items-center gap-1.5 px-3.5 py-2 bg-red-500/5 border border-red-500/10 text-red-400/50 rounded-lg font-montserrat text-[7px] font-bold tracking-widest uppercase hover:bg-red-500/10 hover:text-red-400 transition-all">
                    Delete Order
                  </button>
                </div>
              </div>
    ...
    <transition name="modal">
      <div v-if="deleteConfirm.show" class="fixed inset-0 z-50 flex items-center justify-center p-6">
        ...
        <div
          class="relative bg-[#0F0F0F] border border-white/10 rounded-[24px] p-8 w-full max-w-xs text-center shadow-3xl">
          ...
          <p class="font-montserrat text-[8px] text-white/40 mb-6 uppercase tracking-widest">This action is
            irreversible.</p>
          <div class="flex gap-3">
            <button @click="confirmDelete"
              class="flex-1 bg-red-500 text-white py-3 rounded-xl font-montserrat text-[8px] font-bold uppercase tracking-widest">Delete</button>
            <button @click="deleteConfirm.show = false"
              class="flex-1 border border-white/10 py-3 rounded-xl font-montserrat text-[8px] font-bold uppercase tracking-widest">Retain</button>
          </div>
        </div>
      </div>
    </transition>
